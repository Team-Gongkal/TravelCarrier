...
let TravelerEmail = $(location).attr('href').substring(TravelerEmail.indexOf("/member/")+8);
//계정관리 모달창 활성화 - by 윤아
...
function updateResult(type, data) {
  ...
  if (type == "dia") {
    ...
    $(".traveler_follower").empty();
    if (data == null) return;
    for (var e of data) {
      $(".traveler_follower").append(travelerHtml(e));
    }
  }
